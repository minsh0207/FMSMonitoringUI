...
namespace FMSMonitoringUI.Controlls
{
    public partial class CtrlMonitoring : UserControlRoot
    {
        ...
        #region Event
        public delegate void TroubleEventHandler(string eqpType, int contronNo, string troubleCode);
        public event TroubleEventHandler TroubleOccurEvent = null;

        public void OnTroubleOccurEvent(string eqpType, int contronNo, string troubleCode)
        {
            if (TroubleOccurEvent != null)
            {
                TroubleOccurEvent(eqpType, contronNo, troubleCode);
            }
        }
        #endregion
        ...
        private async Task LoadEntireEqpList()
        {
            try
            {
                ...
                strSQL.Append("      OR (A.eqp_type = 'MIC' AND A.unit_id IS NULL)");
        ...
        private async Task LoadAgingRackData()
        {
            try
            {
                ...
                // Rack 정보
                strSQL.Append(" SELECT aging_type, line, lane, rack_id, status, process_no, fire_flag, use_flag");
        ...
        private void SetData(List<_aging_rack_data> data)
        {
            ...
            // HT Aging
            ht011.SetStatus(rackData[ht011.LinePrefix], _RackStatus);
            ht012.SetStatus(rackData[ht012.LinePrefix], _RackStatus);
            ht013.SetStatus(rackData[ht013.LinePrefix], _RackStatus);
            ht014.SetStatus(rackData[ht014.LinePrefix], _RackStatus);

            // LT Aging #1
            lt011.SetStatus(rackData[lt011.LinePrefix], _RackStatus);
            lt012.SetStatus(rackData[lt012.LinePrefix], _RackStatus);
            lt013.SetStatus(rackData[lt013.LinePrefix], _RackStatus);
            lt014.SetStatus(rackData[lt014.LinePrefix], _RackStatus);

            // LT Aging #2
            lt021.SetStatus(rackData[lt021.LinePrefix], _RackStatus);
            lt022.SetStatus(rackData[lt022.LinePrefix], _RackStatus);
            lt023.SetStatus(rackData[lt023.LinePrefix], _RackStatus);
            lt024.SetStatus(rackData[lt024.LinePrefix], _RackStatus);
        }
        ...
        private void OnCrane_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    int conveyorNo = (int)enEqpType.RTV01;
        ...
        private void Subscription_DataChanged(Subscription subscription, DataChangedEventArgs e)
        {
            ...
            //        }
            //        else if (item.BrowseName == "WaterTank.TrayExist")  // WaterTank1
            //        {
            //            switch (item.GroupNo)
            //            {
            ...
            //            }
            //        }
            //        else if (item.BrowseName == "WaterTank2.TrayExist")  // WaterTank1
            //        {
            //            switch (item.GroupNo)
            //            {
            //                case 1: // HT Aging S/Crane
        ...
        #region ReadTroubleCode
        /// <summary>
        /// Trouble Code를 가져온다.
        /// </summary>
        /// <param name="item"></param>
        /// <returns></returns>
        private void ReadTroubleCode(ItemInfo item, bool trouble)
        {
            List<ReadValueId> cvInfo = _clientFMS[item.ServerNo].ConveyorNodeID[item.SiteNo];
            List<DataValue> data = _clientFMS[item.ServerNo].ReadNodeID(cvInfo);
            ...
            OnTroubleOccurEvent(item.ControlType.ToString(), item.SiteNo, errorNo);
        }
        #endregion
        ...
        private SiteTagInfo ReadWaterTankInfo(int serverNo, int craneNo, int watertankNo)
        {
            ...
            string tagPath = $"WaterTank{watertankNo}.TrayIdL1";
            int trayIdx1 = craneInfo.FindIndex(x => x.UserData.ToString().EndsWith(tagPath));

            tagPath = $"WaterTank{watertankNo}.TrayIdL2";
            int trayIdx2 = craneInfo.FindIndex(x => x.UserData.ToString().EndsWith(tagPath));

            SiteTagInfo tagInfo = new SiteTagInfo();

            if (data[trayIdx1].Value != null)
                tagInfo.TrayIdL1 = data[trayIdx1].Value.ToString();

            if (data[trayIdx2].Value != null)
                tagInfo.TrayIdL2 = data[trayIdx2].Value.ToString();
            ...
            if (this.InvokeRequired)
            {
                await Task.Run(() =>
                {
                    this.Invoke(new MethodInvoker(delegate ()
                    {
                        ...
                        {
                            _ListConveyor[siteInfo.ConveyorNo].UpdateTrackStatus(siteno, item.TrayExist, item.TrayRework, item.EqpStatus, item.TrackPause);
                        }
            ...
            //if (this.InvokeRequired)
            {
                await Task.Run(() =>
                {
                    ...
                    foreach (DataChange change in e.DataChanges)
                    {
                        ...
                        if (item.ControlType == enEqpType.CNV)
                        {
                            if (item.SiteNo == (int)enEqpType.RTV01)
                            {
        ...
        private void ConveyorDataChange(ItemInfo item, string value)
        {
            ...
            if (item.BrowseName == "EquipmentStatus.Status")
            {
                ...
                if (Convert.ToInt16(value) == 4)    // 4: Machine Trouble
                {
                    ReadTroubleCode(item, true);
                    _SubscribeInfo[groupNo].Item[siteNo].PreEqpStatus = 4;
                }
                else
                {
                    // 매번 호출되는것을 막기 위해 추가
                    if (_SubscribeInfo[groupNo].Item[siteNo].PreEqpStatus == 4)
                    {
                        ReadTroubleCode(item, false); 
                    }

                    _SubscribeInfo[groupNo].Item[siteNo].PreEqpStatus = Convert.ToInt16(value);
                }
            }

            if (item.BrowseName == "ConveyorInformation.TrackPause")
            {
                _SubscribeInfo[groupNo].Item[siteNo].TrackPause = Convert.ToBoolean(value);
            }
        ...
        private void StackerCraneDataChange(ItemInfo item, string value)
        {
            ...
            else if ((item.BrowseName == "WaterTank1.TrayExist") || // WaterTank1
                     (item.BrowseName == "WaterTank1.FireSensor"))
            {
                int siteNo = GetWaterTankSiteNo(1, item.GroupNo, item.CraneNo);
                ...
                if (item.BrowseName == "WaterTank1.TrayExist")
                    _SubscribeInfo[groupNo].Item[siteNo].TrayExist = Convert.ToBoolean(value);

                if (item.BrowseName == "WaterTank1.FireSensor")
            ...
            else if ((item.BrowseName == "WaterTank2.TrayExist") || // WaterTank2
                     (item.BrowseName == "WaterTank2.FireSensor"))
            {
                int siteNo = GetWaterTankSiteNo(2, item.GroupNo, item.CraneNo);
                ...
                if (siteNo > 0) 
                {
                    ...
                    if (item.BrowseName == "WaterTank2.TrayExist")
                    {
                        _SubscribeInfo[groupNo].Item[siteNo].TrayExist = Convert.ToBoolean(value);
                    }

                    if (item.BrowseName == "WaterTank2.FireSensor")
                        _SubscribeInfo[groupNo].Item[siteNo].FireSensor = Convert.ToBoolean(value);
